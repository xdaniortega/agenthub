...
export function generateEnvExample(answers: WizardAnswers, chain: ChainConfig): string {
    ...
    const llmKeyLine =
        answers.llmProvider === "claude"
            ? "# Anthropic API key — get yours at https://console.anthropic.com/settings/keys\nANTHROPIC_API_KEY=sk-ant-your_anthropic_api_key_here"
...
export function generateAgentTs(answers: WizardAnswers, systemPrompt?: string): string {
    ...
    if (answers.llmProvider === "claude") {
        return generateClaudeAgentTs(answers, finalSystemPrompt, llmModel);
    }
    return generateOpenAIAgentTs(answers, finalSystemPrompt, llmModel);
}

function generateOpenAIAgentTs(answers: WizardAnswers, systemPrompt: string, llmModel: string): string {
...
`
    ...
    return `/**
 * LLM Agent — OpenAI (${llmModel})
 *
 * Powers your agent's intelligence using the OpenAI API.
 * API key is loaded from OPENAI_API_KEY in .env
 *
 * To switch models: change the 'model' value in chat()
 * To switch providers: replace the OpenAI import and update chat()
 */
...
// OASF skills registered on-chain are prepended here so the LLM is aware of
// its declared capabilities. Edit freely to match your agent's purpose.
...
/**
 * Low-level call to the OpenAI API.
 * @param messages - Full conversation including system prompt
 ...
 * Called by A2A and MCP handlers.
 */
...
function generateClaudeAgentTs(answers: WizardAnswers, systemPrompt: string, llmModel: string): string {
    const streamingCode = answers.a2aStreaming
        ? `
/**
 * Stream a response to a user message (generator function)
...
`
    ...
    return `/**
 * LLM Agent — Claude (${llmModel})
 *
 * Powers your agent's intelligence using the Anthropic API.
...
// Note: Anthropic messages only support 'user' and 'assistant' roles.
...
/**
 * Low-level call to the Anthropic API.
...
${answers.llmProvider === "claude" ? "# Get from https://console.anthropic.com/settings/keys\nANTHROPIC_API_KEY=sk-ant-your_key" : "# Get from https://platform.openai.com\nOPENAI_API_KEY=your_openai_key"}
\`\`\`
